...
const Login: React.FC = () => {
    ...
    const { user, error: authError, loading } = useSelector((state: RootState) => state.auth);
            ...
            <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md flex flex-col justify-between h-full">
                <div>
                    <div className="flex flex-col items-center mb-6">
                        ...
                        <h1 className="text-3xl font-bold text-green-700">QuadraMall</h1>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="relative text-right">
                            <Link to="/forgot-password" className="text-sm text-green-600 hover:underline">Quên mật khẩu?</Link>
                        </div>
                        <button
                            type="submit"
                            disabled={isLoading || loading}
                            className={`w-full bg-green-600 text-white py-2 rounded-lg hover:bg-green-700 transition font-semibold ${isLoading || loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
