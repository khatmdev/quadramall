...
import { set } from 'zod';
import { setSelectedItems } from '@/store/Order/OrderSlice';
  ...
  return (
    <div className="bg-white rounded-xl shadow p-5 w-full max-w-sm">
      ...
      <div className="flex justify-between font-semibold text-base mt-2 mb-4">
        <span>Tổng cộng</span>
        <span>{formatCurrency(totalPrice - discount + tax)}</span>
      </div>
      <div className="bg-gray-50 border rounded-lg p-3 flex items-center justify-between mb-4">
        <div className="flex items-start gap-3">
          <RiDiscountPercentLine className="w-5 h-5 mt-1 text-green-600" />
          <div>
            <p className="font-medium text-sm">Sử dụng mã giảm giá</p>
            <p className="text-xs text-gray-500">Chọn sản phẩm để áp dụng mã</p>
          </div>
        </div>
