#!/bin/bash
echo "🔧 Fixing config structure issue..."

# Check current structure
echo "📁 Current structure:"
ls -la app/ | grep config

if [ -d "app/config" ]; then
    echo "❌ Found app/config/ directory - This is the problem!"
    echo "💾 Backing up app/config/ to app/config_backup/"
    cp -r app/config app/config_backup
    
    # Check if there's config content inside
    if [ -f "app/config/__init__.py" ]; then
        if grep -q "class Settings" app/config/__init__.py; then
            echo "📝 Moving config content from __init__.py..."
            cp app/config/__init__.py app/config.py.new
        fi
    fi
    
    # Remove directory
    echo "🗑️ Removing app/config/ directory..."
    rm -rf app/config
    
    # Create proper config.py
    if [ -f "app/config.py.new" ]; then
        mv app/config.py.new app/config.py
    fi
fi

# Create/update config.py with session management
echo "📝 Creating proper app/config.py..."
...
try:
    ...
    print(f'   - Session TTL: {settings.session_ttl_hours}h')
    print(f'   - Max Context: {settings.max_context_messages}')
